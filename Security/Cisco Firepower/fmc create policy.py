...
pol_response = requests.post(f'{url}{pol_url}', headers=headers, data=json.dumps(payload), verify=False).json()
...
rules_response = requests.post(f'{url}{rules_url}', headers=headers, data=json.dumps(payload), verify=False).json()
print(' ********** RULES CREATED ********** ')
...
del_response = requests.delete(f'{url}{policy_url}', headers = headers, verify=False).json()
